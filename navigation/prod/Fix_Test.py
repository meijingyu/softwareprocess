...
# Happy path 
    ...
    def test100_010_ShouldConstructOmmitted(self):
        ...
        try:
            logTxtFile = open("log.txt",'r')
            entryFirstLine = logTxtFile.readline()
            if "logFile:" not in entryFirstLine:
#                 print "buzai"
                self.fail()
        except:
...
#     def test_setAriesFile(self):
#         myF=F.Fix()
#         myF.setAriesFile("aries.txt")
#         try:
#             logTxtFile = open("log.txt")
#             entryFirstLine = logTxtFile.readline()
#             entrySecondLine = logTxtFile.readline()
#             entryShirdLine = logTxtFile.readline()
#             if "Aries File:" not in entrySecondLine:
#                 print "buzai"   
#         except:
#             self.fail()     
# sad path
...
# Happy path
    def test200_010_ShouldSetNewSightingFile(self):
        ...
        try:
            ...
            entryFirstLine = logTxtFile.readline()
            entrySecondLine = logTxtFile.readline()
            if "Sighting file:" not in entrySecondLine:
                print "buzai"
#                 self.fail()
        except:
            self.fail()
...
# Happy path    
    def test300_010_ShouldReturnLatitudeAndLongitude(self):
        ...
        myF.setAriesFile("aries.txt")
        myF.setStarFile("stars.txt") 
   
#         self.assertEqual(myF.getSightings(), ("-16d44.5","239d13.1"))
